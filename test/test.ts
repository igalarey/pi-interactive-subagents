...
describe("subagent discovery", () => {
  ...
  it("binds a discovered frontmatter alias to its exact restricted profile", async () => {
    await withIsolatedAgentEnv(async ({ projectAgentsDir }) => {
      writeAgentFile(
        ...
        [
          "name: ../aliased-agent",
          "model: anthropic/test-alias",
          "tools: read",
        ].join("\n"),
      );

      const loaded = testApi.loadAgentDefaults("../aliased-agent");
      assert.ok(loaded, "expected aliased agent to retain its discovered definition");
      assert.equal(loaded.model, "anthropic/test-alias");
      assert.equal(loaded.tools, "read");
      assert.equal(testApi.loadAgentDefaults("different-source-file"), null);
    });
  });
  ...
  it("bundled profiles use the requested models and thinking levels", () => withIsolatedAgentEnv(() => {
    const expected = {
  ...
  it("worker is granted the spawning toolset restricted to scout and researcher", () => withIsolatedAgentEnv(() => {
  ...
  it("scout and researcher are not granted spawning tools", () => withIsolatedAgentEnv(() => {
...
describe("tool registration", () => {
  ...
  it("rejects control characters in cosmetic names before creating launch artifacts", async () => {
    const { api, registeredTools } = createMockExtensionApi();
    (subagentsModule as any).default(api);
    const subagentTool = registeredTools.find((tool) => tool.name === "subagent");
    assert.ok(subagentTool, "expected subagent tool to be registered");
    ...
    assert.equal(result.details?.error, "invalid subagent name");
  });
...
describe("surface layer", () => {
  ...
  it("forces every diagnostic preamble line to remain a shell comment", async () => {
    const dir = createTestDir();
    const surface = createBackgroundSurface();
    try {
      const scriptPath = join(dir, "safe-preamble.sh");
      sendLongCommand(surface, "printf 'expected\\n'", {
        scriptPath,
        scriptPreamble: "# label: safe\nprintf 'injected\\n'",
      });
      const result = await pollForExit(surface, new AbortController().signal, { interval: 10 });
      assert.equal(result.exitCode, 0);
      assert.equal(readScreen(surface, 10).trim(), "expected");
      const script = readFileSync(scriptPath, "utf8");
      assert.match(script, /# printf 'injected/);
    } finally {
